...
> : 시작 지점을 지도에 선택 후 계산 버튼을 누르면, 바람을 따라 물체가 어떻게 이동할지 예상 이동 경로를 보여줍니다.    
> : 예측 경로에 “1분, 2분, 3분…”처럼 시간 표시 점이 찍혀, “몇 분 뒤 어디쯤”이 한눈에 들어옵니다.    

> 2. 관측점과 교차 위치 찾기    
> : 관측점을 선택 후 방위각을 입력하면, 2개 이상의 선이 만나는 교차점을 자동으로 계산해 지도에 보여줍니다.    
> : 관측점은 추가/삭제/수정이 가능합니다.    

> 3. 다양한 지도 모드 제공    
> : 위성+지명 / 위성 / OSM 버튼으로 지도를 변경할 수 있으며, 한국형 지도에서는 볼 수 없는 지역까지 정보를 제공합니다.

> 4. 내 위치로 빠르게 이동    
> : 현재 위치를 확인해서 지도가 자동으로 내 위치로 이동합니다.    

> 5. 도형 그리기    
> : 그림판 수준의 쉬운 동작으로 선, 네모, 원, 다각형 등을 그려서 지도에 표시할 수 있습니다.
> : 그린 도형은 색상/굵기/라벨(이름·글자색·배경색)을 바꿀 수 있고, 라벨은 드래그로 위치를 옮길 수 있습니다.    

> 6. 좌표 입력과 변환    
> : 위도·경도와 군사좌표(MGRS) 중 편한 방식으로 위치를 입력할 수 있고, 지도를 클릭하면 해당 좌표가 사이드바에 자동으로 채워집니다.    
...
> 10. 사이드바 열고 닫기    
> : 왼쪽 기능 패널(사이드바)을 접거나 펼칠 수 있고, 사이드바 상태에 따라 지도가림을 최소화 합니다.    

> 11. 바람 데이터 자동 새로고침(선택 사항)    
> : 바람 데이터를 일정 주기(초 단위)로 다시 불러와서 경로를 자동 갱신합니다.

> 12. 도형 한 번에 지우기
> : 그리기 도구로 지도가 복잡해졌다면 버튼 한 번으로 그려둔 도형들을 깔끔하게 정리(삭제)할 수 있습니다.    
